...
import net.ddns.evcalyptus.bonus.*;
...
public class JPanelCloseMoney extends JPanel implements JPanelView, BeanFactoryApp {
    ...
    private DataLogicTeamBonus dl_TB;
    private DataLogicIndivBonus dl_IB;
    private DataLogicBonusConfig dl_BC;
    ...
    public void init(AppView app) throws BeanFactoryException {
        ...
        dl_IB = (DataLogicIndivBonus) m_App.getBean("net.ddns.evcalyptus.bonus.DataLogicIndivBonus");
        dl_TB = (DataLogicTeamBonus) m_App.getBean("net.ddns.evcalyptus.bonus.DataLogicTeamBonus");
        dl_BC = (DataLogicBonusConfig) m_App.getBean("net.ddns.evcalyptus.bonus.DataLogicBonusConfig");
    ...
    private void m_jCloseCashActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_m_jCloseCashActionPerformed
        ...
        if (res == JOptionPane.YES_OPTION) {
            ...
            try {
            	...
            	//l.info("m_App.getActiveCashIndex() = " + m_App.getActiveCashIndex());
                    java.util.List bonuspeople = dl_IB.bonusListPerson(m_App.getActiveCashIndex());
                    //l.info("m_dlSystem.bonusListPerson().toString()=" + m_dlSystem.bonusListPerson(m_App.getActiveCashIndex()).toString());
                    //l.info("bonuspeople = " + bonuspeople);
                    for (int i = 0; i < bonuspeople.size(); i++) {
                        //l.info("bonuspeople.size =" bonuspeople.size());
                        //l.info("i=" + i);
                    
                        AppUser user = (AppUser) bonuspeople.get(i);
                        Object [] money_person = new Object [] {m_App.getActiveCashIndex(), user.getId()};
                        //need to check if money_person combo already inserted
                        if (dl_IB.isMoneyPersonInserted(money_person) < 1) {
                            Double bonussumsales = dl_IB.bonusSumPayments(money_person);
                            Double bonussumbonus = dl_IB.bonusSumBonus(money_person);
                            // check if daily sales plan is fulfilled.  If not, IndivBonus = 0
                            if (bonussumsales < dl_BC.mapIndivPlan.get(user.getRole())) bonussumbonus = 0.0;
                            
                            dl_IB.bonusInsertPerson(dl_IB.bonusGetPerson(money_person), bonussumsales, bonussumbonus);
                        }
                    }
                }
            ...
            //run the teambonus logic
            
            try {
                //need to check that money is in the tbsession
                boolean isTBASM = dl_TB.isTeamBonusActiveSessionMoney(m_App.getActiveCashIndex()); 
                
                if (isTBASM) {//(dl_TB.isTeamBonusConfigHost(m_App.getProperties().getHost())){
            	
                    
            
                	try {
                    	/*in any case update ActiveCashIndex session numbers to TeamBonusActiveSession
            		 * at Close of Money*/
                             dl_TB.updateTeamBonusSessionNumbers(m_App.getActiveCashIndex(), DataLogicBonusConfig.TB_FROM_SALES);
            		/*
                     * = public final boolean isTeamBonusSessionLastMoney() - 
                     * "SELECT COUNT(C.MONEY) FROM CLOSEDCASH C, TEAMBONUSSESSIONMONEY T WHERE C.MONEY = T.MONEY AND C.DATEEND IS NULL" 
                     * check if all MONEY in the current TEAMBONUSSESSION have been closed.   
                     * */
                        
                        if (dl_TB.isTeamBonusSessionLastMoney(m_App.getActiveCashIndex())) {
                        
                        //if session plan is not fulfilled, teambonus = 0
                        
                            if(!dl_TB.isTeamPlanOK(dl_TB.getTeamBonusActiveSessionId(), DataLogicBonusConfig.TB_SESSION_PLAN)) dl_TB.setTeamBonusActiveSessionToZero();
                            
                            /*close TeamBonusActiveSession.  That includes calculating and updating bonuses per user*/
                            dl_TB.closeTeamBonusActiveSession();
                            dl_TB.setTeamBonusActiveSession();
            ...
            try {
                ...
                //After closing of prev tbsession and money and opening of new tbsession and money,  see if we can insertTBActiveSessionMoney
                if (!dl_TB.isTeamBonusActiveSessionMoney(m_App.getActiveCashIndex()) & 
                                        dl_TB.isNoMoneyOfHostInActiveTBSession(m_App.getProperties().getHost())){ 
            				//only one money/activeCash per host is allowed in tbsession
            				//otherwise tbsession will never close with multiple hosts
            				dl_TB.insertTeamBonusActiveSessionMoney(m_App.getActiveCashIndex());
            			}
