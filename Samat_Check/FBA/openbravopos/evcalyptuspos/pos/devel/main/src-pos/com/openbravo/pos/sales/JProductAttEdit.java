...
public class JProductAttEdit extends javax.swing.JDialog {
    ...
    private SentenceFind getstockbyproduct;
    private SentenceFind getstockbyattrsetinst;
    ...
    private void init(Session s) {
        ...
        attvaluesSent = new PreparedSentence(s, "SELECT VALUE FROM ATTRIBUTEVALUE WHERE ATTRIBUTE_ID = ?",
                ...
                SerializerReadString.INSTANCE);
        /*
        getstockbyproduct = new PreparedSentence (s, 
            "SELECT UNITS FROM STOCKCURRENT WHERE PRODUCT = ?",
            SerializerWriteString.INSTANCE,
            SerializerReadDouble.INSTANCE);
        
        getstockbyattrsetinst = new PreparedSentence (s,
            "SELECT UNITS FROM STOCKCURRENT WHERE PRODUCT = ? AND ATTRIBUTESETINSTANCE_ID = ?",
            new SerializerWriteBasic(Datas.STRING, Datas.STRING),
            SerializerReadDouble.INSTANCE);
           */
    ...
    /*
    public double getCurrentStockByAttr(String product_id, String attributeinstance_id) throws BasicException{
        return (double) getstockbyattrsetinst.find(product_id, attributeinstance_id);
    }
    
    public double getCurrentStockByProd (String product_id) throws BasicException {
        return (double) getstockbyproduct.find(product_id);
    }
    */
