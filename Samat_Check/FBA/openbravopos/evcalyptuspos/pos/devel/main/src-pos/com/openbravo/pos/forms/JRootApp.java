...
import net.ddns.evcalyptus.bonus.DataLogicTeamBonus;
...
public class JRootApp extends JPanel implements AppView {
    static final Logger l = Logger.getLogger(JRootApp.class.getName());
    ...
    //  added for JPanelSettingsDefault
    ...
    private String m_sDefaultProductMargin;
    ...
    public boolean initApp(AppProperties props) {
        ...
        m_sDefaultProductMargin = m_propsdb.getProperty("defaultproductmargin");
        if (m_sDefaultProductMargin == null) {
            m_sDefaultProductMargin = "000";
            m_propsdb.setProperty("defaultproductmargin", m_sDefaultProductMargin);
            m_dlSystem.setResourceAsProperties(sPropertySettings, m_propsdb);
        }
        
        // instantiate DataLogicTeamBonus
        
        dl_TB = (DataLogicTeamBonus) getBean("net.ddns.evcalyptus.bonus.DataLogicTeamBonus");
        /*
        Check if the current host is eligible for teambonus. If not, ignore TeamBonus logic.  
        If yes, proceed with TeamBonus logic
        */
        if (dl_TB.isTeamBonusConfigHost(m_props.getHost()) == true) {
            Object[] tb_activesession = new Object[3]; 
            try {
                tb_activesession = dl_TB.getTeamBonusActiveSession();
            } catch (BasicException e) {
            ...
            /*
            get the current session (if there is one) from TEAMBONUSSESSION. 
            if there is none, call setTeamBonusSession() and insertTeamBonusSession(Object[] tbsession)
            */
            if (tb_activesession == null){
                try {
                    dl_TB.setTeamBonusActiveSession();
                    if (!dl_TB.isTeamBonusActiveSessionMoney(getActiveCashIndex()) & dl_TB.isNoMoneyOfHostInActiveTBSession(getProperties().getHost())){ 
        				//only one money/activeCash per host is allowed in tbsession
        				//otherwise tbsession will never close with multiple hosts
        				dl_TB.insertTeamBonusActiveSessionMoney(getActiveCashIndex());
        			}
                } catch (BasicException e) {
        	l.info("BasicException e: " + e);
        	//TODO : implement MessageInf msg = new MessageInf(MessageInf.SGN_NOTICE, AppLocal.getIntString("message.cannotinsertteambonus"), e);
            ...
            /*
            check if the open TEAMBONUSSESSION already contains current money=m_App.getActiveCashIndex(), 
            if yes just proceed, if no, do insertTeamBonusSessionMoney;
            */
            boolean isTBASM = false;
            try {
                isTBASM = dl_TB.isTeamBonusActiveSessionMoney(getActiveCashIndex());
            } catch (BasicException e) {
        	l.info("BasicException e: " + e);
        	//TODO : implement MessageInf msg = new MessageInf(MessageInf.SGN_NOTICE, AppLocal.getIntString("message.cannotinsertteambonus"), e);
                //msg.show(this);
    ...
    @Override
    public String getDefaultProductMargin() {
        return m_sDefaultProductMargin;
    }
    ...
    private void initComponents() {
        ...
        jLabel1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/com/openbravo/images/logo_forte.png"))); // NOI18N
        jLabel1.setText("<html><center>FortePOS автоматизированная система управления торговой точкой.</center></html>");
